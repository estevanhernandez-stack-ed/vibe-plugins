...
1. **Tenant walls (hard).** A configurable list of path prefixes that are never indexed and never surfaced. `Marcus\` is seeded from the estate keystone and is not a preference: a walled path is a refusal, not a warning. Asserted by test.
   ...
   **The wall needs one enforcement point, not one per producer.** This build produced four separate wall defects: the defaults exported and enforced by nothing; the wall test passing against an empty fixture estate; case-sensitive matching letting `marcus` through on a case-insensitive filesystem; and remote enumeration surfacing a walled repo name because it never consulted the wall at all. Every new surface that produces repo records forgot it. The fix is structural: every producer filters, **and** a single shared guard asserts the invariant on the way out, so the next producer inherits enforcement instead of rediscovering the bug. A guarantee re-implemented at each call site is a guarantee that will be missed at one of them.
...
3. **Fork and vendored-checkout filter.** *Added after the 2026-08-11 cowpath run, which this filter's absence would have sunk.* A repo whose history the user barely authored is somebody else's code sitting inside the estate boundary, and surfacing it as "you already built this" is the product being wrong about its one claim. Two cheap signals from `git shortlog -sn` and `git rev-list --count`:
   - **Authorship ratio.** Commits by the user's identities over total commits, both counted over the same ref scope. Mixing `shortlog --all` against `rev-list --count HEAD` yields ratios above 1.0, which is proof the two numbers describe different populations. Below `minAuthorshipRatio` (0.75), classify as `foreign`.
   - **Identities are discovered, never listed.** A hardcoded author list cannot survive a developer who commits under several git identities across machines and repo configs. Count instead how many distinct estate repos each identity authors in, and treat any appearing in **two or more** as the user's. Measured against the real 86-repo estate: `Estevan Hernandez` 58 repos, `estevanhernandez-stack-ed` 35, a stray machine identity 21, another alt 4, the user's own CI bot 2 — against the Microsoft author of the forked PowerToys at 1, and `Claude` at 1. An upstream author in a fork appears in exactly one repo; that gap is the entire discriminator, and it needs no tuning. Explicitly configured `authors` union with the discovered set and can never subtract from it, the same floor principle as the tenant walls.
   - **No commit-density rule.** An earlier draft also classified by commits-per-file, on the theory that a checkout with thousands of files and a handful of commits was cloned rather than written. Measured against the real estate it fired on 13 repos, every one a legitimate single-commit initial import at authorship ratio 1.00, and it caught nothing authorship had not already caught. It detects young repos, not forks. Removed — a signal with no unique true positives and thirteen false ones is not a conservative safeguard, it is a silent eraser.
