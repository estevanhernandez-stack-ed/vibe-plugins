...
| `vibe-lingual` | `Vibe-Lingual` | `plugins/vibe-lingual` |
| `vibe-access` | `vibe-access` | `plugins/vibe-access` |

`vibe-insights` uses the `url` source type (whole-repo over explicit HTTPS, manifest at root, no subpath — switched from `github` on 2026-06-09 because that type resolves SSH clone URLs and fails with publickey-denied for users without GitHub SSH keys); the rest use `git-subdir` with a `path`. Account for both branches when scripting against the manifest — the one-liner above already does. Don't reintroduce the `github` source type here.
